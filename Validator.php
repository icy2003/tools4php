...
class Validator
{
    ...
    public static function rules($data, $rules, $option = null)
    {
        ...
        define('VALIDATOR_FRAMEWORK', $optionArray['framework']);
        ...
        $return = true === $optionArray['safe'] ? array_intersect_key($returnData, array_flip($safeFieldArray)) : $returnData;
        if (!empty($error)) {
            $return += array(
                'error' => $error,
            );
    ...
    {
        return array_key_exists('isEmpty', $rule) ? self::filterValidator(array(
                $value), 0, array(
    ...
    {
        //手机号规则暂时不需要那么严格
        return self::matchValidator($data, $field, $rule + array('pattern' => '/^1\\d{10}$/'));
    }

    protected static function emailValidator($data, $field, $rule)
    {
        return self::matchValidator($data, $field, $rule + array('pattern' => '/^[\w\-\.]+@[\w\-]+(\.\w+)+$/'));
    }
    ...
    protected static function encodeValidator($data, $field, $rule)
    {
        $value = self::value($data, $field);
        $isEmpty = self::isEmpty($value, $rule);
        return !$isEmpty ? htmlspecialchars($value, ENT_QUOTES, 'utf-8') : null;
    }
